...
class Solution{
public:
    // Approach 1: whenever root is NULL, create root and return it.
    Node* helper(Node *root, int &val)
    {
        ...
        if(root->data > val) root->left = helper(root->left, val);
        else if(root->data < val) root->right = helper(root->right, val);

    }

    // Approach 2: At each step divide the pre array in two parts left and right, and process them both
    Node* insert(int mini, int maxi, int pre[], int start, int end)
    {
        if(start == end) return NULL;
        Node *temp = NULL;
        if(pre[start] < maxi && pre[start] > mini)
        {
            temp = newNode(pre[start]);
            int i = start+1;
            for(; i < end; i++)
            {
                if(pre[i] > pre[start]) break;
            }
            temp->left = insert(mini, temp->data, pre, start+1, i);
            temp->right = insert(temp->data, maxi, pre, i, end);
        }
        return temp;
    }
    Node* post_order(int pre[], int size)
    {
        ...
        root->left = insert(INT_MIN, root->data, pre, 1, i);
        root->right = insert(root->data, INT_MAX, pre, i, size);
        return root;
    }
};
